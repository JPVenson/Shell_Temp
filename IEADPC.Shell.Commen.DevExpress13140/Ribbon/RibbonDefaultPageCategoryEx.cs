...
namespace IEADPC.Shell.Commen.DevExpress13140.Ribbon
{
    public class RibbonDefaultPageCategoryEx : RibbonDefaultPageCategory
    {
        ...
        {
            var moduleSorterService = VisualMainWindow.ApplicationProxy.ServicePool.GetSingelService<IModuleSorterService>();
            var arrModules =
                moduleSorterService.Sort(modules.Select(moduleSorterService.GenerateDescriptor)
            ...
            foreach (ModuleLayoutDescriptor mod in arrModules)
            {
